...
Rectangle {
    ...
    function openTab(tabIndex, serverUrl, token) {
        ...
        if (serverUrl !== undefined && typeof serverUrlField !== 'undefined') serverUrlField.text = serverUrl
        if (token !== undefined && typeof tokenField !== 'undefined') tokenField.text = token
        else if (typeof tokenField !== 'undefined') tokenField.text = appSettings.token
                    ...
                    Flickable {
                        ...
                        ColumnLayout {
                            ...
                            Repeater {
                                ...
                                model: settingsWindow.localServersList.filter(function(s) { return s.enabled })
                                delegate: ColumnLayout {
                                    id: serverDelegateRoot
                                    property string serverName: modelData ? modelData.name : "Unknown Server"
                                    width: librariesColLayout.width
                                    spacing: 12
                                    ...
                                    Repeater {
                                        ...
                                        delegate: RowLayout {
                                            ...
                                            spacing: 15
                                            CheckBox {
                                                objectName: "libraryCheckbox_" + model.ratingKey
                                                checked: !!librariesTabCol.localLibrariesMap[model.ratingKey]
                                                onToggled: {
                                                    var map = Object.assign({}, librariesTabCol.localLibrariesMap)
                                                    if (checked) { map[model.ratingKey] = { "type": model.type, "title": model.title, "serverName": serverDelegateRoot.serverName } }
